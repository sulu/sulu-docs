...
Selecting the ``Authenticator app (TOTP)`` method in the profile opens a guided setup: the user
scans the shown QR code with an authenticator app (or enters the secret manually) and confirms the
setup with a generated verification code. The method is only activated after a valid code was
confirmed.
...
configured yet. A user that already uses the authenticator app method is not affected.
