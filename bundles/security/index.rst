...
Sulu allows to use two-factor authentication via the scheb/2fa packages. The following methods
and features are available:

- ``scheb/2fa-email``: a verification code is sent to the email address of the user
- ``scheb/2fa-totp``: a verification code is generated by an authenticator app
  (e.g. Google Authenticator, Microsoft Authenticator, ...)
- ``scheb/2fa-google-authenticator``: an alternative to ``scheb/2fa-totp`` that is also TOTP
  based and works with the same authenticator apps - installing one of the two packages is
  sufficient
- ``scheb/2fa-backup-code``: one-time backup codes to log in when the configured method
  is not available
- ``scheb/2fa-trusted-device``: allows the user to skip the second factor on trusted devices

To enable it, the base package and the packages for the wanted methods need to be installed
into the project via composer:

.. code-block:: bash

    composer require scheb/2fa-bundle scheb/2fa-email scheb/2fa-totp scheb/2fa-backup-code scheb/2fa-trusted-device
...
Afterwards, the scheb/2fa bundle needs to be configured to enable the installed methods
...
Selecting the ``Authenticator app (TOTP)`` or ``Google Authenticator`` method in the profile opens
a guided setup: the user scans the shown QR code with an authenticator app (or enters the secret
manually) and confirms the setup with a generated verification code. The method is only activated
after a valid code was confirmed.

Backup Codes
~~~~~~~~~~~~

When the ``scheb/2fa-backup-code`` package is installed and enabled, a ``Generate backup codes``
action is shown in the profile for every active two-factor method. Each generated code can be used
once instead of the regular verification code, for example when the authenticator app is not
available. The codes are stored as password hashes and are only shown once on generation.
Generating a new set invalidates all previously generated backup codes.

Reset Two-Factor Authentication
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Administrators can reset the two-factor authentication of a user, for example when the user lost
access to the authenticator app. This is possible via the ``Reset Two-Factor authentication``
toolbar button on the permissions tab of the user or via the following command:
...
Two-factor authentication over email can be enforced for users with an email address matching a
configured pattern in the ``config/packages/sulu_security.yaml`` file:
