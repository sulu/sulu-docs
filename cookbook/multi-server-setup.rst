...
In the Media Storage only original files are stored. The image formats are generated by Sulu the first time they got
requested. That mechanic (reverse proxy cache) storages like S3, Blob Storage, ... do not support.
So for every server the image will be regenerated once. Aslong as no media are removed that might not be
a problem. You can per server configure ``bin/console sulu:media:format:cache:cleanup`` as a cronjob to make sure
that a image format is also removed on all servers.

Still in a kubernetes / container setups where you might loose the ``public/uploads/media`` everytime a container restart,
you need to find different ways to cache images.
You can make the ``public/uploads/media`` directory a mounted volume which is shared between all containers (might not be good for performance).
Better use some kind of CDN (cloudflare, fastly, ...) or a reverse proxy cache via nginx to cache generate images formats.

If you configure a CDN / reverse proxy correctly you not require the cronjob and can disable the image format filesystem cache completely via:
...
Never disable the image format cache if you do not use any CDN or reverse proxy cache which caches files a very long time.
