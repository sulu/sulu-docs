...
Additional features, which can be provided with an DataProvider:
...
.. list-table::
    ...
    * - append(QueryBuilder $queryBuilder, $alias, $locale, $options = [])
      - Additional select, where or joins can be added to the query to match
        given options. The options can be generated by the data-provider and
        can contain for example additional filter parameter.
